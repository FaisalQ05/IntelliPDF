...
export const useCreateChat = () => {
  ...
  return useMutation({
    mutationFn: async (documentId: string) => {
      const res = await post<ApiResponse<Chat>>("/pdf-chat/chats", {
        documentId,
      });
      if (!res.success) throw new Error(res.message);
      return res.data;
...
export const useDeleteChat = () => {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      await del<ApiResponse<void>>(`/pdf-chat/chats/${id}`);
      return id;
      // if (!res.success) throw new Error(res.message);
      // return res.data;
    },
...
export const useGetMessages = (chatId: string | null) => {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await get<ApiResponse<ChatMessage[]>>(
        `/pdf-chat/chats/${chatId}/messages`
      );
      if (!res.success) throw new Error(res.message);
      return res.data;
    },
